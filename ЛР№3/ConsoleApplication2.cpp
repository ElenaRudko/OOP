...
public:
	...
	void static readFromFile()
	{
		...
		try {
			...
			//T1 firstName, T1 klas, T2 counture, T2 year, T3 c
...
{
	/*T1 qvalue = q->firstName.substr(0, q->firstName.size());
	q->firstName = p->firstName.substr(0, p->firstName.size());
	p->firstName = qvalue;*/
	...
	/*T2 qvalue3 = q->counture;
	q->counture = p->counture;
	p->counture = qvalue3;
...
int main()
{
	...
	example.AddTail("Анастасия", "Беларусь", 1, 2000, "Скрипка");
	example.AddTail("Вячеслав", "Беларусь", 2, 1995, "Фано");
	example.AddTail("Кирилл", "Россия", 1, 1999, "Гитара");
	example.AddTail("Егор", "Украина", 6, 2000, "Скрипка");
	example.AddTail("Володя", "Беларусь", 3, 1997, "Барабан");
	example.AddTail("Яна", "Беларусь", 2, 2000, "Фано");
